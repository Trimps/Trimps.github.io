...
function resetGame(keepPortal) {
	...
	if (keepPortal){
		...
		for (var cItem in allChallenges){
			if (game.challenges[cItem] && game.challenges[cItem].allowSquared)
				game.challenges[cItem].highestSquared = allChallenges[cItem].highestSquared;
		}
	}
	else {
