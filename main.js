...
"use strict";
// Useful for linters. Note: is not accurate, as it was generated from
// main = Intersection(updatesDepends, configDepends)
// updates + config = Union(updatesDepends - main, configDepends - main)
//let kongregate, game, LZString, message, tooltip, setFormation, squaredConfig, clearQueue, refreshMaps, getNextPrestigeValue, canAffordBuilding, cancelPortal, displayGoldenUpgrades, boneTemp, TTimeAccel, lookingAtCurrentChallenge, addResCheckMax, checkHousing, getMaxForResource, simpleSeconds, getFuelBurnRate, unlockJob, buildBuilding, gameTimeout, createHeirloom, canAffordCoordinationTrimps, addNewSetting, givePumpkimpLoot, updateSkeleBtn, heirloomsShown, displayPortalUpgrades, prestigeEquipment, recycleAllExtraHeirlooms, recalculateHeirloomBonuses, getNextPrestigeCost, updateDecayStacks, abandonDaily, addSpecials, enablePlayFab, unlockEquipment, displaySelectedHeirloom, canAffordJob, getPlayerModifier, toggleGeneticistassist, canAffordTwoLevel, fireMode, repeatClicked, getTooltipJobText, cancelTooltip, givePresimptLoot, dailyModifiers, updateGoodBar, updateTalentNumbers, displayNature, getMagmiteDecayAmt, buildGrid, game, initTalents, getHighestTalentTier, isSaving, buyBuilding, purgeBionics, refundQueueItem, romanNumeral, voidBuffConfig, getScientistLevel, updateGeneratorInfo, displayTalents, unlockMapStuff, toggleAutoStorage, message, unlockUpgrade, activateTurkimpPowers, getScientistInfo, getAvgLootSecond, updateForemenCount, prettify, scaleToCurrentMap, drawGrid, toggleAutoPrestiges, checkAchieve, countDailyWeight, updateAllPerkColors, rewardResource, startDaily, resetOnePortalRewards, calculateDamage, unlockFormation, updateAllBattleNumbers, playFabId, getGeneratorUpgradeHtml, tooltip, hideFormations, createMap, getPlayerCritDamageMult, setGather, updateGeneratorUpgradeHtml, scaleNumberForBonusHousing, calculateTimeToMax, autoSave, tooltipUpdateFunction, updateBalanceStacks, getNextTalentCost, hideBones, resolvePow, setNonMapBox, toggleVoidMaps, selectedPreset, resetAdvMaps, getEmpowerment, addHelium, goldenUpgradesShown, getPortalUpgradePrice, getMinutesThisPortal, getPlayFabLoginHTML, getAvailableGoldenUpgrades, giveSingleAchieve, shiftPressed, swapClass, getHighestPrestige, checkChallengeSquaredAllowed, getDailyHeliumValue, countChallengeSquaredReward, getRandomIntSeeded, setNewCraftItem, countHeliumSpent, planetBreaker, levelEquipment, disableSaving, calculateMaxAfford, getDailyTimeString, readingDaily, toggleAutoGolden, isNumberBad, scaleLootLevel, setEmpowerTab, calculatePercentageBuildingCost, getGoldenFrequency, updateMapCredits, getPlayerCritChance, numTab, resetGame, addQueueItem, achievementCompatibilityUnlock, unlockMap, filterMessage, filterTabs, updateLabels, displayPerksBtn, toggleSetting, getAchievementStrengthLevel, calculateAchievementBonus, isObjectEmpty, removeQueueItem, getPsString, checkEndOfQueue, updatePs, drawAllBuildings, drawAllUpgrades, goRadial, getBarColorClass, strength, ga, checkButtons, countAlertsIn, PlayFab, postMessages, unlockTooltip, trimpAchievementsOpen;
...
function load(saveString, autoLoad, fromPf) {
	...
	if (oldVersion === 1.0){
		...
		for (var portItem in game.portal){
			portUpgrade = game.portal[portItem];
	...
	if (oldVersion < 2.7){
		...
		if (game.global.totalHeliumEarned > 0){
			var totalHelium = 0;
			for (item in game.portal){
				if (game.portal[item].locked) continue;
				portUpgrade = game.portal[item];
	...
	if (getAchievementStrengthLevel() <= 0) {
		...
		for (item in game.goldenUpgrades){
...
function recalculateHeirloomBonuses(){
	var item;
	for (item in game.heirlooms.Staff) game.heirlooms.Staff[item].currentBonus = 0;
	for (item in game.heirlooms.Shield) game.heirlooms.Shield[item].currentBonus = 0;
	var mod;
	if (game.global.StaffEquipped){
		for (item in game.global.StaffEquipped.mods){
			mod = game.global.StaffEquipped.mods[item];
			game.heirlooms.Staff[mod[0]].currentBonus = mod[1];
		}
	}
	if (game.global.ShieldEquipped){
		for (item in game.global.ShieldEquipped.mods){
			mod = game.global.ShieldEquipped.mods[item];
...
function replaceMod(confirmed){
	...
	if (!confirmed && game.options.menu.boneAlerts.enabled == 1) {
		...
		if (oldName != "Empty"){
			if (mod[3] > 0){
				cost = countPriceOfUpgrades(setupDummyHeirloom(heirloom, mod), mod[3]);
...
function naturePurchase(doing, spending, convertTo){
	var empowerment, cost;
	if (doing == 'upgrade'){
		cost = getNextNatureCost(spending);
		empowerment = game.empowerments[spending];
	...
	if (doing == "stackTransfer"){
		...
		if (empowerment.retainLevel >= 80)
			return;
		cost = getNextNatureCost(spending, true);
		if (empowerment.tokens < cost) return;
		empowerment.tokens -= cost;
            ...
            if(direction !== undefined) {
                for(var i = 0; i < length; i++) {
                    ...
                    switch(direction) { //check the 5 cells in the direction of which we're going
                                        ...
                                        //-----
                        case 0:
                            for(x = vector.x; x <= vector.x + 1; x++) { //iterating over the 6 cells
                                for(y = vector.y - 1; y <= vector.y + 1; y++) {
                        ...
                        case 1:
                            ...
                            break;
                        case 2:
                            for(x = vector.x - 1; x <= vector.x + 1; x++) {
                                for(y = vector.y; y <= vector.y + 1; y++) {
                                    if(x == vector.x && y == vector.y)
                                        continue;
...
function getGeneratorUpgradeHtml(){
		...
		var item;
		for (item in game.generatorUpgrades){
		...
		for (item in game.permanentGeneratorUpgrades){
...
function updateGeneratorUpgradeHtml(){
	...
	var item, elem, upgrade, cost, text, state;
	for (item in game.generatorUpgrades){
		elem = document.getElementById('generatorUpgrade' + item);
		if (elem == null) continue;
		upgrade = game.generatorUpgrades[item];
		cost = upgrade.cost();
		text = item + "<br/>" + upgrade.upgrades;
		elem.innerHTML = text;
	...
	for (item in game.permanentGeneratorUpgrades){
		elem = document.getElementById('generatorUpgrade' + item);
		if (elem == null) continue;
		upgrade = game.permanentGeneratorUpgrades[item];
		cost = upgrade.cost;
		text = item + "<br/>";
...
function getRandomBadGuy(mapSuffix, level, totalCells, world, imports, mutation) {
	...
	if (!mapSuffix && !force) {
		var chance = 0.35 * (1 / (100 - 1 - (3 * imports.length)));
...
function drawGrid(maps) { //maps t or f. This function overwrites the current grid, be carefulz
	...
	if (maps){
		...
		if (map.size % cols === 0) rows = map.size / cols;
			...
			rows = ((map.size - (cols * cols)) > cols) ? (cols + 2) : (cols + 1);
		}
	}
	var width = (100 / cols);
    var counter = 0;
...
function easterEggClicked(){
	...
	if (roll <= 0.84 || (roll > 0.92 && game.global.runningChallengeSquared)){
		...
		amt = rewardResource(reward, (rewardRoll / 1.5), game.global.eggLoc);
		startText += prettify(amt) + " " + reward + "!";
	}
	else if (roll <= 0.89){
		amt = Math.round(game.global.world / 4);
		if (amt <= 0) amt = 1;
    ...
    if (cell.health <= 0 || !isFinite(cell.health)) {
		...
		if (game.global.challengeActive == "Daily"){
			...
			if (typeof game.global.dailyChallenge.karma !== 'undefined'){
				...
				maxStack = dailyModifiers.karma.getMaxStacks(game.global.dailyChallenge.karma.strength);
			...
			if (typeof game.global.dailyChallenge.toxic !== 'undefined'){
				...
				maxStack = dailyModifiers.toxic.getMaxStacks(game.global.dailyChallenge.toxic.strength);
			...
			if (typeof game.global.dailyChallenge.rampage !== 'undefined'){
				...
				maxStack = dailyModifiers.rampage.getMaxStacks(game.global.dailyChallenge.rampage.strength);
...
function switchFormToRecovery(){
	var title = document.getElementById("playFabLoginTitle");
	var emailInput;
	if (title != null) emailInput = document.getElementById("playFabEmailHidden");
	if (emailInput != null) emailInput.style.display = 'block';
...
function playFabRegisterPlayFabUser(){
	...
	try {
		...
		if (saveLogin) {
			storePlayFabInfo(requestData.Username, requestData.Password);
...
function playFabSaveCheck(){
	...
	console.assert(typeof PlayFab === 'undefined' || typeof PlayFab.ClientApi === 'undefined');
	var requestData = {
