...
function load(saveString, autoLoad, fromPf) {
    ...
    for (var a in game) { //global, resources, jobs, buildings, upgrades, triggers, equipment, settings, options
		...
		if (a == "options" && savegame.options){
			...
			if (savegame.options.menu.GeneticistassitTarget && savegame.options.menu.GeneticistassistTarget.disableOnUnlock) game.options.menu.GeneticistassistTarget.disableOnUnlock = true;
...
function buildGrid() {
	...
	if (game.global.world >= 181 || (game.global.challengeActive == "Corrupted" && game.global.world >= 60)){
		var startMod = (game.global.challengeActive == "Corrupted") ? 60 : 181;
		var possible = Math.floor((game.global.world - startMod) / 3) + 2;
...
function nextWorld() {
	...
	if (game.global.world == 181 && game.global.challengeActive != "Corrupted") tooltip("Corruption", null, 'update');
